...
CardMaker::CardMaker( SamplePairVec Samples, const ColorMap colorMap, const Double_t inLumi, const DblVec puweights, const TString indir, const TString outdir, const Bool_t Blind, const TString type){
  ...
  for (SamplePairVecIter iter = Samples.begin(); iter != Samples.end(); ++iter){
    if ( (*iter).second == 0 )      {fBkgNames.push_back((*iter).first);}  // non-res background
    else if ( (*iter).second == 1 ) {fResNames.push_back((*iter).first);}  // resonant background
...
void CardMaker::MakeCards(){
  ...
  for (SamplePairVecIter iter = Samples.begin(); iter != Samples.end(); ++iter){
    CardMaker::ApplyCommonSelection( (*iter).first );// apply common selection to each sample
  } 

}// end CardMaker::MakeCards


void CardMaker::ApplyCommonSelection( const TString fSample ){
...
void CardMaker::SetTChainBranchAddresses(){
...
};// end CardMaker::SetTChainBranchAddresses
