...
def makePlots(infile,outdir,mvatype):
  fin     = ROOT.TFile.Open(infile)
  fout    = ROOT.TFile.Open(outdir+'plotsMVAstudies_'+mvatype+'.root','RECREATE')

  ROOT.gStyle.SetOptStat(0)
  ROOT.gStyle.SetOptTitle(0)

  getPlots(fin,outdir,mvatype)

def getPlots(fin,outdir,mvatype):
  addtree = ''
  if mvatype == 'DNN':
    addtree = 'dataset/'

  # --- Pick out different trees
  MVATree   = addtree+'Method_'+mvatype+'/'+mvatype+'/'
  ...
  h_train_ROC = ROOT.TH1D(fin.Get(MVATree+'MVA_'+mvatype+'_trainingRejBvsS'))
  h_test_ROC  = ROOT.TH1D(fin.Get(MVATree+'MVA_'+mvatype+'_rejBvsS'))
  ...
  scut = 'classID>0.5'
  bcut = 'classID<0.5'
  TrainTree.Project("h_train_S",mvatype,scut)
  TrainTree.Project("h_train_B",mvatype,bcut)
  TrainTree.Project("h_test_S",mvatype,scut)
  TrainTree.Project("h_test_B",mvatype,bcut)  

  # --- Draw each plot with right styling 
  plotROCcurve(h_train_ROC,h_test_ROC)                  # ROC 
  plotCorrelations(h_corr_sig,'S')                      # CorrS
  plotCorrelations(h_corr_bkg,'B')                      # CorrB
  plotClassifier(h_train_S,h_train_B,h_test_S,h_test_B) # Classifier 

def plotROCcurve(h_train_ROC,h_test_ROC):
  ...
  c1.Print(outdir+'plot_ROC_'+mvatype+'.png')
  c1.Print(outdir+'plot_ROC_'+mvatype+'.pdf')
  h_train_ROC.Write()
  h_test_ROC.Write()

def plotCorrelations(h_corr,which):
  c2 = ROOT.TCanvas("c2"+which,"",600,600)
  c2.cd()
  
  h_corr.Draw("COLZ")
  c2.Print(outdir+'plot_Corr'+which+'_'+mvatype+'.png') 
  c2.Print(outdir+'plot_Corr'+which+'_'+mvatype+'.pdf') 
  h_corr.Write()
 
def plotClassifier(h_train_S,h_train_B,h_test_S,h_test_B):
  ...
  h_train_S.GetXaxis().SetTitle('Classifier, '+mvatype)
  ...
  l3.Draw('SAME')
  c3.Print(outdir+'plot_Classifier_'+mvatype+'.png')
  c3.Print(outdir+'plot_Classifier_'+mvatype+'.pdf')
  h_train_S.Write()
  h_train_B.Write()
  h_test_S.Write()
  h_test_B.Write()
...
if __name__ == "__main__":
  mvatype = 'DNN'
  outdir  = '~/www/Plots/MonoHgg2017/MVAplots/' 

  infile = ''
  if (mvatype == 'SVM'):
    infile = 'MVALearningOutput.root'
  if (mvatype == 'DNN'): 
    infile = 'TMVA_DNN.root' 

  print("Making plots for : %s" %mvatype)
  makePlots(infile,outdir,mvatype)  
