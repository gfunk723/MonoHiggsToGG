...
void makeLimitPlotNicer(){
  ...
  TString inDir  = "/afs/cern.ch/work/m/mzientek/private/FitDirectory/EXO_7_4_15/src/HiggsAnalysis/CombinedLimit/ntuples4fit_BARY/";
...
void run(TString inDir, TString outDir, int type){
  ...
  outfile->cd();
  gc_exp->SetName("expected_baryonic");
  gc_obs->SetName("observed_baryonic");
  gc_exp->Write();
  gc_obs->Write();
...
void getMassPoints( std::vector<TString> & names ){
  ...
  int mZp[] = {10,20,30,50,75,100,150,200,250,300,400,500,600,700,900,1200,1500,2000,2500,3000,4000,5000,6000,8000,10000}; 
  int mDM[] = {1,5,10,20,40,60,100,125,150,175,200,250,300,350,400,450,500,600,700,800,900,1000};
  int nZp = sizeof(mZp)/sizeof(*mZp);
  int nDM = sizeof(mDM)/sizeof(*mDM);
