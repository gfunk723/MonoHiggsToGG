...
EventList readEventListSinglePhoAnalyzer(char const* _fileName) {
...
class NewSinglePhoAnalyzer : public edm::EDAnalyzer {
  
public:
  
  explicit NewSinglePhoAnalyzer(const edm::ParameterSet&);
  ~NewSinglePhoAnalyzer();
...
NewSinglePhoAnalyzer::NewSinglePhoAnalyzer(const edm::ParameterSet& iConfig):
  // collections
...
NewSinglePhoAnalyzer::~NewSinglePhoAnalyzer() { 
...
void NewSinglePhoAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
...
//double NewSinglePhoAnalyzer::applyJetCorrection(const flashgg::Jet &y)
//    {
//      double jec_adjust = 1.;
...
int NewSinglePhoAnalyzer::sortedIndex(const unsigned int vtxIndex, const unsigned int sizemax, const Ptr<flashgg::DiPhotonCandidate> diphoPtr ) {
...
bool NewSinglePhoAnalyzer::geometrical_acceptance(float eta1, float eta2)
{
  float ae1(fabs(eta1));
  float ae2(fabs(eta2));
...
void NewSinglePhoAnalyzer::beginJob() {
  ...
  listCSC     = readEventListSinglePhoAnalyzer("/afs/cern.ch/user/c/crovelli/public/monoH/metFilters/csc2015_Dec01.txt");
  listEEbadSC = readEventListSinglePhoAnalyzer("/afs/cern.ch/user/c/crovelli/public/monoH/metFilters/ecalscn1043093_Dec01.txt");
  listHadronTrackRes= readEventListSinglePhoAnalyzer("/afs/cern.ch/user/s/soffi/public/MonoHgg/MetFilters/badResolutionTrack_Jan13.txt");
  listMuonBadTrack = readEventListSinglePhoAnalyzer("/afs/cern.ch/user/s/soffi/public/MonoHgg/MetFilters/muonBadTrack_Jan13.txt");
...
void NewSinglePhoAnalyzer::endJob() { 
...
void NewSinglePhoAnalyzer::initTreeStructure() {
...
void NewSinglePhoAnalyzer::SetPuWeights(std::string puWeightFile) {
...
float NewSinglePhoAnalyzer::GetPUWeight(float pun) {
...
bool NewSinglePhoAnalyzer::isGammaPresel( float sceta, float pt, float r9, float chiso, float hoe) {
...
bool NewSinglePhoAnalyzer::rediscoveryHLT(float sceta, float pt, float r9,float sieie, float pfIso,float trkSum03 ){
  bool isEB=false;
  bool isEE=false;
  bool HLTok=false;
...
double NewSinglePhoAnalyzer::getChargedHadronEAForPhotonIso(float eta){
...
double NewSinglePhoAnalyzer::getNeutralHadronEAForPhotonIso(float eta) {
...
double NewSinglePhoAnalyzer::getGammaEAForPhotonIso(float eta) {
...
bool NewSinglePhoAnalyzer::LeadPhoTriggerSel(float eta, float hoe, float r9, float sieie, float phoiso, float pt){
  bool passes = false;
...
bool NewSinglePhoAnalyzer::SubLeadPhoTriggerSel(float eta, float hoe, float r9, float sieie, float phoiso, float chiso, float pt){
  bool passes = false;
  if (fabs(eta)<1.4442 && hoe < 0.1){
...
int NewSinglePhoAnalyzer::passSieieCuts(float sceta, float sieie){
  int passes = 1;
  if (fabs(sceta)<1.4442 && sieie>0.0102) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) && sieie>0.0268) passes = 0;
  return passes;
}
int NewSinglePhoAnalyzer::passCHisoCuts(float sceta, float chiso, float pt){
  int passes = 1;
  if (fabs(sceta)<1.4442 && chiso>1.37) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) && chiso>1.10) passes = 0;
  return passes;
}
int NewSinglePhoAnalyzer::passNHisoCuts(float sceta, float nhiso, float pt){
  int passes = 1;
...
int NewSinglePhoAnalyzer::passPHisoCuts(float sceta, float phiso, float pt){
  int passes = 1;
  if (fabs(sceta)<1.4442 && phiso > (0.28 + 0.0053*pt)) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) && phiso > (0.39 + 0.0034*pt)) passes = 0;
  return passes;
}
int NewSinglePhoAnalyzer::passHoeCuts(float sceta, float hoe){
  int passes = 1;
...
// tight working point
int NewSinglePhoAnalyzer::passTightSieieCuts(float sceta, float sieie){
  int passes = 1;
  if (fabs(sceta)<1.4442 && sieie>0.0100) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) && sieie>0.0268) passes = 0;
  return passes;
}
int NewSinglePhoAnalyzer::passTightCHisoCuts(float sceta, float chiso, float pt){
  int passes = 1;
  if (fabs(sceta)<1.4442 && chiso>0.76) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) && chiso>0.56) passes = 0;
  return passes;
}
int NewSinglePhoAnalyzer::passTightNHisoCuts(float sceta, float nhiso, float pt){
  int passes = 1;
...
int NewSinglePhoAnalyzer::passTightPHisoCuts(float sceta, float phiso, float pt){
  int passes = 1;
  if (fabs(sceta)<1.4442 && phiso > (0.08 + 0.0053*pt)) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) &&  phiso > (0.16 + 0.0034*pt)) passes = 0;
  return passes;
}
int NewSinglePhoAnalyzer::passTightHoeCuts(float sceta, float hoe){
  int passes = 1;
  if (fabs(sceta)<1.4442 && hoe>0.05) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) &&hoe>0.05) passes = 0;
  return passes;
...
// loose working point
int NewSinglePhoAnalyzer::passLooseSieieCuts(float sceta, float sieie){
  int passes = 1;
  if (fabs(sceta)<1.4442 && sieie>0.0102) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) && sieie>0.0274) passes = 0;
  return passes;
}
int NewSinglePhoAnalyzer::passLooseCHisoCuts(float sceta, float chiso, float pt){
  int passes = 1;
  if (fabs(sceta)<1.4442 && chiso>3.32) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) && chiso>1.97) passes = 0;
  return passes;
}
int NewSinglePhoAnalyzer::passLooseNHisoCuts(float sceta, float nhiso, float pt){
  int passes = 1;
...
int NewSinglePhoAnalyzer::passLoosePHisoCuts(float sceta, float phiso, float pt){
  int passes = 1;
  if (fabs(sceta)<1.4442 && phiso > (0.81 + 0.0053*pt)) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) &&  phiso > (0.83 + 0.0034*pt)) passes = 0;
  return passes;
}
int NewSinglePhoAnalyzer::passLooseHoeCuts(float sceta, float hoe){
  int passes = 1;
  if (fabs(sceta)<1.4442 && hoe>0.05) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) &&hoe>0.05) passes = 0;
  return passes;
...
bool NewSinglePhoAnalyzer::testPhotonIsolation(int passSieie, int passCHiso, int passNHiso, int passPHiso, int passHoe, int passEleVeto){
...
bool NewSinglePhoAnalyzer::isGammaSelected( float rho, float pt, float sceta, float r9, float chiso, float nhiso, float phoiso, float hoe, float sieie, bool passElectronVeto) {
...
int NewSinglePhoAnalyzer::effectiveAreaRegion(float sceta) {
...
float NewSinglePhoAnalyzer::getSmearingValue(float sceta, float r9, int syst){
...
float NewSinglePhoAnalyzer::getScalingValue(int sampleID, float sceta, float r9, int runNumber, int syst){
...
float NewSinglePhoAnalyzer::applyEnergySmearing(float ptUncorr, float sceta, float r9, int run){
...
float NewSinglePhoAnalyzer::applyEnergyScaling(int sampleID, float ptUncorr, float sceta, float r9, int run){
  float Scaling	= getScalingValue(sampleID, sceta, r9, run, 0 );
  float pt = ptUncorr*Scaling; 
  return pt;
}

float NewSinglePhoAnalyzer::getPtCorrected(float ptUncorr, float sceta, float r9, int run, int sampleID){
  float pt;
...
DEFINE_FWK_MODULE(NewSinglePhoAnalyzer);
